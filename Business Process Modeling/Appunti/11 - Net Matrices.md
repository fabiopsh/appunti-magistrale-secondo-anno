...
Ripasso minimo di algebra lineare che useremo: un **vettore** su un insieme finito $E = \{e_1, \dots, e_n\}$ è una mappa $v : E \to \mathbb{Q}$ (o $\mathbb{N}, \mathbb{Z}$), scritta $v = [v(e_1), \dots, v(e_n)]$; useremo la stessa notazione per vettori riga e colonna. Il **prodotto scalare** di due vettori è $x \cdot y = \sum_i x_i y_i$. Confronti tra vettori: $v \le w$ se $v(e) \le w(e)$ per ogni $e$; $v < w$ se $v \le w$ ma diversi; $v \prec w$ se $v(e) < w(e)$ per *ogni* $e$.

---

## L'idea: la marcatura come vettore, il firing come somma

Il primo passo è ovvio: una marcatura $M : P \to \mathbb{N}$ è già un vettore, di lunghezza $n = |P|$. Per esempio $M_0 = [4, 0, 1, 0, 0]$ dice "4 token in $p_1$, uno in $p_3$".
...
Con la matrice, lo scatto di una singola transizione diventa una **somma di vettori**. Se $M \xrightarrow{t} M'$, allora $M' = M + \vec{t}$, dove $\vec{t}$ è la colonna di $t$ nella matrice di incidenza. Per esempio, dalla marcatura $M_0 = [4,0,1,0,0]$ lo scatto di $t_3$ (colonna $[0,0,-1,1,0]$) porta a $M_1 = [4,0,0,1,0]$.
...
>
> Data una sequenza $\sigma \in T^\star$, il suo **Parikh vector** $\vec{\sigma} : T \to \mathbb{N}$ associa a ogni transizione $t$ il **numero di sue occorrenze** in $\sigma$. Per una singola transizione, $\vec{t}$ è il vettore con un $1$ nella posizione di $t$ e $0$ altrove. Definizione ricorsiva: $\vec{\epsilon} = \mathbf{0}$ e $\overrightarrow{\sigma t} = \vec{\sigma} + \vec{t}$.
...
> *Dimostrazione (induzione sulla lunghezza di $\sigma$).* **Base** ($\sigma = \epsilon$): $\vec{\sigma} = \mathbf{0}$, quindi $M' = M$, banalmente vero. **Passo** ($\sigma = \sigma' t$): sia $M \xrightarrow{\sigma'} M'' \xrightarrow{t} M'$. Allora $M' = M'' + \mathbf{N}\cdot\vec{t} = (M + \mathbf{N}\cdot\vec{\sigma'}) + \mathbf{N}\cdot\vec{t} = M + \mathbf{N}\cdot(\vec{\sigma'} + \vec{t}) = M + \mathbf{N}\cdot\vec{\sigma}$. $\blacksquare$
...
>
> Se $M \xrightarrow{\sigma} M'$, allora $M + L \xrightarrow{\sigma} M' + L$ per **qualsiasi** vettore $L$. (Vale anche per sequenze infinite.)
>
> *Idea:* per induzione, usando la marking equation a ogni passo: se $M''$ abilita $t$, allora $M'' + L$ abilita $t$ (ha *almeno* gli stessi token), e lo scatto produce $M'' + L + \mathbf{N}\cdot\vec{t} = M' + L$.
...
>
> Se un sistema è **bounded** e $M \in [M_0\rangle$ con $M \supseteq M_0$, allora $M = M_0$.
>
> *Idea:* se fosse $M = M_0 + L$ con $L \ne \mathbf{0}$, per monotonicità potremmo raggiungere $M_0 + kL$ per ogni $k$, accumulando token senza limite — contraddicendo la boundedness.
>
...
>
> **Conseguenza:** se una sequenza $\sigma$ si può eseguire un numero arbitrario di volte, allora $\sigma$ **produce almeno tante risorse quante ne consuma** (altrimenti, ripetendola, prima o poi un place andrebbe sotto zero). Combinato col corollario precedente, si ha: $\sigma$ ripetibile all'infinito $\iff$ $M \subseteq M'$.
